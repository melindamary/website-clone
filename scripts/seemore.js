...
async function fetchMoviesByGenreAndLanguage(genreIds, lang) {
	let url = baseUrl;
	if (genreIds) {
...
async function createSection() {
	...
	if (lang) {
		document.getElementById("heading").innerHTML = "Movies in English";
	}

	console.log("Genre IDs:", genreIdArray);
	...
	const movies = await fetchMoviesByGenreAndLanguage(genreIdArray, lang);
	...
	try {
		...
		if (genre) {
			...
			if (genre.name === "Documentary") {
				document.getElementById("heading").innerHTML = `${genre.name}`;
			} else {
				document.getElementById("heading").innerHTML = `${genre.name} Movies`;
			}
		}
	} catch (error) {
