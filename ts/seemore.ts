import { apiKey, url as baseUrl, imageBaseUrl } from "./config";
import { Movie } from "./types/movies";
import { CardHtmlElement } from "./types/movies";

async function fetchMoviesByGenreAndLanguage(genreIds: number[], lang: string) {
	let url = baseUrl;
	if (genreIds) {
...
function createMovieCard(movie: Movie) {
	...
	function showCardDetails(movie: Movie, card: CardHtmlElement) {
...
async function getGenreName(genreId: number, mediaType = "movie") {
	try {
