...
const playTrailer = (): void => {
  ...
  const movieId: string | null = urlParams.get('movieId');
  ...
  let trailerVideo = document.getElementById("trailer-video") as HTMLIFrameElement;
  if (!trailerVideo) {
  ...
  fetch(`https://api.themoviedb.org/3/movie/${movieId}/videos?`, apiOptions)
      ...
      .then((response) => {
          let trailers = response.results.filter((video: { type: string }) => video.type === "Trailer");
